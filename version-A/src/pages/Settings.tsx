...
import { Download, Trash2, Shield, Globe, RefreshCw, CloudSync } from "lucide-react";
...
import { isBackendOnline } from "../services/auth-v2";
...
export default function Settings() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold text-slate-900">
          <CloudSync size={16} /> Synchronisation backend
        </h3>
        <p className="mb-3 text-sm text-slate-600">
          Poussez vos données locales (IndexedDB) vers le serveur FinTrack (Express + PostgreSQL).
          Utile après avoir travaillé hors-ligne ou lors de la première connexion.
        </p>
        <Button
          variant="outline"
          disabled={syncing}
          onClick={async () => {
            ...
            try {
              ...
              if (!online) return alert("Backend hors-ligne. Impossible de synchroniser.");
              const r = await syncAll(user!.id);
              const total = Object.values(r).reduce((a, b) => a + b, 0);
              alert(`✅ Synchronisation terminée — ${total} entité(s) poussée(s).\nComptes: ${r.accounts} · Catégories: ${r.categories} · Transactions: ${r.transactions} · Budgets: ${r.budgets} · Objectifs: ${r.goals} · Récurrences: ${r.recurring}`);
            } finally {
              setSyncing(false);
            }
          }}
        >
          <CloudSync size={14} /> Synchroniser mes données
        </Button>
      </Card>
