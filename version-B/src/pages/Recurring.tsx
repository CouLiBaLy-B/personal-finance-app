...
export default function Recurring() {
  ...
  async function handleDelete(id: string) {
    if (!confirm("Supprimer cette récurrence ? (les transactions déjà générées seront conservées)")) return;
    await db.recurring.delete(id);
  }

  async function handleSync() {
    const n = await processRecurring(userId);
    alert(n > 0 ? `${n} transaction(s) générée(s).` : "Aucune nouvelle transaction à générer.");
  }
