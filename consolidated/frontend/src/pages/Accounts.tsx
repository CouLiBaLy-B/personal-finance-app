...
export default function Accounts() {
  ...
  async function handleDelete(id: string) {
    const ok = await confirmDialog({
      ...
      description: "Toutes les transactions liées à ce compte seront également supprimées. Cette action est irréversible.",
...
function NewAccountModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    setName("");
    setInitialBalance("0");
    toast.success("Compte créé", `${name.trim()} a été ajouté.`);
    onClose();
  }
